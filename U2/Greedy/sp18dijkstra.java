...
public class sp18dijkstra {
    ...
    static List<Integer> last(int[][] arr, int n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            visited[mini] = true;
            for (int j = 0; j < n; j++) {
                if (!visited[j] && arr[mini][j] != 0 && dist[mini] != Integer.MAX_VALUE
                        && dist[mini] + arr[mini][j] < dist[j]) {
                    dist[j] = dist[mini] + arr[mini][j];
                }
            }
        }
